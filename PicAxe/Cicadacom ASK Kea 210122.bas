#Picaxe 08M2
#Com 7          'Set com Port for programming and serial terminal monitoring
#Terminal 2400  'Directive to set the Serial Terminal ro 4800 Baud
'#No_Data       'Directive to prevent data table download
...
do 
  ...
  Bat_Volts:					'Send the Battery mV to Cayenne
    ...
    gosub Post_Data
  ' gosub Post_Data ' Only need second sending when using radio.  Wired is more reliable.
  ...
  Pi_Hibernator:
    if pin2 = 0 OR time > 250 then 	'IF Pi Data Pin low Pi has shut down !
        ...
        sleep 30									'Allow Pi to Tidy and Complete ShutDown
        ...
        for w0 = 1 to 1500				'Wait for 300 = 10 min ~ 1500 = 50 minutes
          serout 0,N2400,(13,10,"System is uAmp Sleeping",9,#w0,9,#pin4,9,#pin2)	'Echo Sleeping State
  ...
  Pi_Start_Up:
   if pin4 = 0 then								'IF Pi is already Shut Down then...
      ...
      if w12 > 4000 then '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''5100 then							'IF Battery = Good then...
         ...
         serout 0,N2400,(13,10,"Pi DC Power Switched ON",9,#pin4,9,#pin2)		'Echo Pi State
         time = 0									'Reset the Time 
      endif
   endif  
