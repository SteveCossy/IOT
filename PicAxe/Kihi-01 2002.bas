...
Sensor_Data_1:
 pokesfr %10001100, %00001000		'WPU Power ON selected Pin(s)
 readadc10 1, w1				'Read Voltage on pin
 pokesfr %10001100, %00000000		'WPU Power OFF selected Pin(s)
 w0 = 1024 - w1				'Invert to get 'Moisture Level'
 b9 = "A"					'Tx data
 gosub RF_Tx:
...
Read_Sensors:
 ...
 readtemp12 2, w2					'Read pin 1 DS18B20
...
Sequence_Data_3:				'Cycle Time Counter
 inc time					'Bump up the 'tme' counter variable by one count
 b9 = "C"					'Tx data
 w0 = time
 gosub RF_Tx:
...
Bat_Check:					'Occasionally 
 ...
 w3 = 262 - w3 * 15 + 4001		'Calculate rough mVolts 
 b9 = "D"					'Tx battery mVolts
 w0 = w3 / 2 * 2
 gosub RF_Tx:

Low_Volt_Standby:				'Go to sleep if volts are too low to save battery
 if w3 < 3000 then			'If the battery drops to 3 Volts then it is flat
  sleep 60					'Take a LONG sleep
